...
- Those doing longitudinal and time series analysis will be interested in the  `stats`and the ‘tseries’ packages. The packages `survival` and `eha` deal with event history and survival analysis, whereas  `plm` are designed for panel data and fixed/random effects models.

- Using R for creating maps  is  now common among social sicentists and geographers with packages such as `rmaps`, `sp`, `rgdal`,  `rgeos` and `ggmaps`
...
- The UK Data Service has produced training material about creating maps in R, as part of a introduction to [mapping crime data](https://github.com/UKDataServiceOpen/Crime_Data_in_R?tab=readme-ov-file)
