...
It originates in the ‘S’ and ‘S Plus’ languages developed during the 1970s
and 1980s. It has a large audience in the science and statistics communities and
is increasingly used in the social and data sciences for teaching and research. 
...
(17,672 at the time of writing this guide)  is one of the strengths of R.
...
Although R can perform most of the analyses available in tradtional statistical software
...
that there are often several ways to carry out the same analyses in R, each one with its
advantages and downsides. Users can also easily produce  publication quality
output from R thanks to its integration with the Markdown (now Quarto) and LaTeX document
presentation systems. R graphs can also easoily be imported into html, MS Word or
LibreOffice documents.

By contrast with other statistical software,  the R interface is rather minimal
and consist merely of a terminal.  In line with  languages such a
Python or C, R users tend to access it via an programming interface, or Integrated
Development Environment (IDE).  In this  guide we will be using the R Studio development
...
that as open-source software, R and its packages are mainly developed by volunteers,
