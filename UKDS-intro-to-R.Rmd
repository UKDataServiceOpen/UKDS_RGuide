...
mydata<-read.spss("data/bsa_spss/bsa2019_poverty_open.sav",to.data.frame = T,max.value.labels=5)%>%select(RSex,skipmeal,Married,Poverty1,HEdQual3,NatFrEst)%>%slice(1:50)
...
levels with the `factor()` function. It is good practice to carry out such manipulation in a newly created variable. 
...
```{r 3.2}
 bsa<-read.spss("data/8849spss_73589506FAA92AEFC9646C684CD2427012F90FA9F6541F3BE46E4BE1A047987C_V1/bsa2017_open_enviropol.sav", to.data.frame = TRUE,
...
- Further regression analysis: the `glm()` command  can be used for fitting a
  ...
  linear and non-linear multilevel models, also known as
  mixed models.
...
- Users interested in latent variable modelling will be interested in the
  factanal() command. Other resources are provided in the ‘poLCA’ (Latent Class
  Analysis), ‘ltm’ (Latent Trait modelling), ‘sem’ (Structural equation
  modelling)   `Lavaan` provides functions for structural equation modelling
